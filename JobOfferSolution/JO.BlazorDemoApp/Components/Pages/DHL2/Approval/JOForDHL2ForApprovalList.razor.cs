...
namespace JO.BlazorDemoApp.Components.Pages.DHL2.Approval
{
    public partial class JOForDHL2ForApprovalList
    {
        [Inject] private IJODetailsService JODetailsService { get; set; } = default!;
        [Inject] private IAccountService AccountService { get; set; } = default!;

        private static readonly int[] ForApprovalWorkFlowIds = [13];
        private static readonly int[] SendBackWorkFlowIds = [10];
        private static readonly int[] ApprovedWorkFlowIds = [7, 8, 9];
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            joDboxCandidates = await JODetailsService.GetForApprovalReviewJODboxCandidates(userId, 13); //For Division Head L2 Approval
