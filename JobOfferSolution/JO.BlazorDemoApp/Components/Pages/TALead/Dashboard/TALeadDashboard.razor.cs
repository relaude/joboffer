namespace JO.BlazorDemoApp.Components.Pages.TALead.Dashboard
{
    public partial class TALeadDashboard
    {
        ...
        {
            var candidateLink = await CandidateService.GetTALeadCandidateLink(candidate);
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            var candidates = await JODetailsService.GetVwTALeadDboxCandidates();
            ...
            countForReview = trackableJobOffers.Count(offer => offer.WorkFlowId == 14);
            countSendBack = trackableJobOffers.Count(offer => offer.WorkFlowId == 10);
            countForApproval = trackableJobOffers.Count(offer => offer.WorkFlowId is 5 or 6 or 7 or 13);
            ...
            var eligibleCandidates = allCandidates
                .Where(candidate => (candidate.CSGId is 1 or 109) || candidate.DivisionId == 3)
                .ToList();
        ...
        private void GoToJobOffers() => Navigation.NavigateTo(JO.Service.Constants.JORoutes.TALead.JobOfferTracker);

        private void GoToCandidates() => Navigation.NavigateTo(JO.Service.Constants.JORoutes.TALead.Candidates);

        private void GoToJOforReview() => Navigation.NavigateTo(JO.Service.Constants.JORoutes.TALead.JOForReview);

        // Review precedes approval; TA Lead review is workflow 14 and approval combines 5, 6, 7 and 13.
